...
The categories $\mathrm{Mon}(\mathbb{D})$ and $\mathrm{Comon}(\mathbb{D})$ come naturally equipped with functors to $\mathcal{D}_0$, defined on objects and morphisms simply by applying $p$ to the underlying 1-cells and 2-cells respectively. It turns out that the interaction between the $\otimes$ and $\odot$ compositions in the 2-fold double category structure is precisely what is needed to lift $\odot$ to $\mathrm{Mon}(\mathbb{D})$ and to lift $\otimes$ to $\mathrm{Comon}(\mathbb{D})$. In this way, we can define double categories $\mathbb{M}\mathrm{on}(\mathbb{D})$ and $\mathbb{C}\mathrm{omon}(\mathbb{D})$, both having $\mathcal{D}_0$ as vertical category.
...
In this same way, we can define the horizontal composition of two 1-cells $(X,\epsilon,\delta)$ and $(Y,\epsilon',\delta')$ in $\mathbb{C}\mathrm{omon}(\mathbb{D})$ to be a comonad with underlying horizontal 1-cell $X\otimes Y$.
...
\begin{definition}\label{Def:Bimonad}
	A \emph{bimonad} in a 2-fold double category $\mathbb{D}$ is a monad in $\mathbb{C}\mathrm{omon}(\mathbb{D})$, or equivalently a comonad in $\mathbb{M}\mathrm{on}(\mathbb{D})$. We can define a category of bimonads in $\mathbb{D}$ as
	\[
