...
\begin{lemma}
	There is a double functor $R\colon\FF{D}_{\otimes}\to\mathbb{D}$ whose behavior on 2-cells is
	\[
	\begin{tikzcd}[column sep=large]
		C \rar[tick][domA]{(E_1,\eta_1,\epsilon_1)} \dar[swap]{F}  & C \dar{F} \\
		D \rar[tick][codA,swap]{(E_2,\eta_2,\epsilon_2)} & D
	...
	\end{tikzcd}
	\]
	and a double functor $L\colon\FF{D}_{\odot}\to\mathbb{D}$ whose behavior on 2-cells is
	\[
	\begin{tikzcd}[column sep=large]
		C \rar[tick][domA]{(E_1,\eta_1,\epsilon_1)} \dar[swap]{F}  & C \dar{F} \\
		D \rar[tick][codA,swap]{(E_2,\eta_2,\epsilon_2)} & D
...
\end{lemma}
\begin{corollary}
	$R$ and $L$ respectively induce functors $\Mon(\FF{D})\to\Mon(\mathbb{D})$ and $\Comon(\FF{D})\to\Comon(\mathbb{D})$.
\end{corollary}
